...
> **状态机与并发控制：围绕预约、支付、主动取消及到期关闭构建订单生命周期状态机，基于数据库复合 CAS（Compare-And-Set）实现条件化状态迁移，并以持久化请求状态、资源版本和可靠任务处理 MQ 重复消费及多事件并发竞争，避免提前关单、重复支付、重复释放与非法状态跳转。**

上一篇：[从一次预约请求理解异步削峰与双层防超卖](./01-异步预约与防超卖.md)。上一篇回答“订单怎么创建且不超卖”，本篇从订单创建成功以后继续，回答“多个事件争着修改同一订单时，谁能生效”。

**源码基线**：GitHub `main` 分支的 v4 可靠性实现，核心代码版本为 `bb089e4258f6c0eee13e778644b657aacf95ceff`。本篇使用与第一篇相同的当前源码，不根据原型项目或旧注释补写能力。
...
- [3. 10:08 用户点击支付：顺着源码走一次成功路径](#section-9)
  ...
  - [3.5 事务提交以后，只需刷新 Redis 请求投影](#section-14)
...
- [9. CAS 成功以后还有事务：为什么不能停在“影响行数为 1”？](#section-41)
  ...
  - [9.4 成功事务后的 Redis 投影失败，不该再取消一遍](#section-45)
...
| `resource_version` | 0 | 直接预约取得的初始名额代次 |
| MySQL 请求事实 | `RESERVED` | 预约成功，等待支付，不是“已经付款” |
| Redis 请求投影 | `RESERVED` 或暂时缺失 | 五分钟加速数据，不承担最终状态 |
...
[ReservationStatus.java](../src/main/java/com/citypass/utils/ReservationStatus.java) 中的 `PROCESSING`、`RESERVED`、`WAITLISTED`、`EXPIRED` 等字符串，主要服务于 `tb_reservation_request` 的持久化请求结果，也复用于候补记录与短期 Redis 投影。尤其需要分清：
...
| `EXPIRED` | 超时释放后的请求事实终态；候补来源订单还会同步把关联候补写成 EXPIRED；**不是订单新增的第七个状态** |
...
stateDiagram-v2
    ...
    UNPAID --> CANCELLED: 主动取消，cancelAndPromote 提交
    UNPAID --> CANCELLED: 到期关闭，expireAndPromote 复合 CAS 提交
    note right of PAID
        当前没有实现核销或退款迁移
    end note
...
直接预约 `source=DIRECT`：状态改成 2、记录 `pay_time` 后，不再修改库存，不释放 Redis Claim，也不创建新的 MQ 消息或可靠任务；同一事务还把 `tb_reservation_request` 从 `RESERVED` 单调推进为 `PAID`。

若 `source=WAITLIST`，同一事务还会把对应候补记录从 `OFFERED` 改为 `ACCEPTED`，并检查影响行数为 1；不满足就抛异常，让订单支付迁移一并回滚。这是保证订单与关联候补状态一起变化的接口边界，完整候补生命周期留到第三篇。

**💡 为什么这里还要事务？**

对候补来源的订单，只改成已支付、却没有同步更新请求事实与关联候补状态，会让三张表对同一次业务给出不同结论。所以支付 CAS 不是孤立一条语句，必要的数据库关联变化需要一起提交。

<a id="section-14"></a>

### 3.5 事务提交以后，只需刷新 Redis 请求投影

事务方法成功返回 `true` 时，MySQL 请求事实已经是 `PAID`。外层 `payOrder()` 随后只执行短期投影：

```java
projectRequest(orderId, userId, ReservationStatus.PAID);
return Result.ok("支付成功");
```

实际源码用三元表达式区分成功失败，上面是成功路径的等价摘述。`projectRequest()` 尽力写 owner 与 `reservation:request:10001=PAID`，TTL 都是 5 分钟；异常只记警告，不会把数据库支付事务改回去。

| 数据 | 支付前 | 正常支付后 |
| --- | --- | --- |
| MySQL 订单 status | 1 | 2 |
| MySQL `pay_time` | 未设置 | 本次应用时间 |
| MySQL 请求事实 | `RESERVED` | `PAID`，与订单同事务 |
| MySQL 库存 | 已被预约占用后的值 | 不变 |
| Redis stock | 已被预约占用后的值 | 不变 |
| Redis holders / claim | 用户仍持有名额 | 不变，不释放 |
| Redis 请求投影 | `RESERVED`、已过期或缺失 | 尽力写 `PAID`，5 分钟 |
| 原来的超时任务 / TIMEOUT | 已登记或已发送 | 支付代码不撤回、不删除它 |

这里“事务后更新”只指 Redis 投影：持久化请求状态已经在 `payUnpaidOrder()` 事务内更新。源码没有为这一步实现 `afterCommit` 回调框架，也不需要靠投影写成功来证明支付事实。

若返回 `false`，Service 返回 `Result.fail("订单不存在、资格已过期或状态不允许支付")`，不写 Redis 的 PAID。若数据库已提交而 Redis 不可用，投影失败只记录日志，HTTP 仍可返回支付成功；查询接口直接读取 MySQL 请求事实与订单。
...
```java
transactionalService.cancelAndPromote(
        orderId, UserHolder.getUser().getId(), offerMinutes);
```

主动取消和系统超时现在有两个明确入口：`cancelAndPromote()` 与 `expireAndPromote()`。它们都进入内部 `releaseOrPromote(..., requireExpired)`，但前者传 `requireExpired=false`，后者传 `true`。**状态竞争就在这笔释放事务里面**，不是先在外面取消成功再来处理名额。
...
如果查询的是本人未支付订单，事务接着用 `selectByIdForUpdate()` 锁定该活动的库存行。候补入队和名额释放都先经过这一个活动级串行点，防止“队列刚判空就有人入队、同时又把库存回池”。但刚刚查到未支付仍只是当时的判断，不能阻止另一线程此刻支付；真正的订单终态仍由后面的条件 UPDATE 决定。

<a id="section-18"></a>

### 4.3 真正争取释放权的是这条条件 UPDATE

[ReservationTransactionalService.releaseOrPromote()](../src/main/java/com/citypass/service/impl/ReservationTransactionalService.java) 的主动取消核心条件等价于：
...
等价语义：`UPDATE ... SET status=4 WHERE id=:orderId AND status=1 AND user_id=:expectedUserId`。`user_id` 条件由非空 `expectedUserId` 动态追加，系统超时路径不追加它，而是追加到期条件；第 5 节单独展开。
...
本篇只需记住：**RELEASED 与 PROMOTED 都要先通过同一张旧订单的 CAS 门槛**。旧请求事实也在同一事务变成 `CANCELLED`；补位时新请求事实变成 `RESERVED`。具体如何严格锁定队首、如何递增 `resourceVersion` 完成 Resource Handoff（名额转交），第三篇再展开。
...
`publishReleaseResult()` 名字里虽然有 publish，**这里并不发送 MQ，也不再承担持久化状态迁移**。取消事务已经将旧请求事实写为 `CANCELLED`；若发生补位，也已将新请求事实写为 `RESERVED`。该方法只尽力刷新对应 Redis 投影并记录日志。

真正清理 Redis holder / claim 或回补 Redis stock，由可靠任务扫描器稍后执行，不是这个方法直接做。数据库事务已提交与 Redis 已完成释放，不是同一个时刻。


### 4.6 注解：关于 releaseOrPromote(...)

整个方法的核心设计：

    释放订单时，不是简单地“库存 +1”。系统先锁住活动库存行，原子关闭旧订单，再优先尝试把释放出的名额直接交给候补队列中的下一位用户；只有没有有效候补时，才把 MySQL 库存加回去，并通过可靠任务异步恢复 Redis 库存。这样可以避免名额先回公共库存后又被其他请求抢走，从而保证候补补位的确定性。
...
上一篇已经读到 `createOrder()` 最后调用 `enqueueTimeout(orderId, expiresAt)`。它在创建订单的同一数据库事务里登记：
...
| `payload` | `10001` |
| 初始状态 | `PENDING` |
| `next_retry_time` | 订单真实 `offer_expire_time` |

直接预约和候补补位的截止时间都取 `min(now + offerMinutes, activityEnd)`。因此任务在资格真正到期前不会成为可领取任务，活动只剩 3 分钟时也不会错误给出 15 分钟资格。等待发生在 MySQL 任务的调度时间上，不再依赖 RocketMQ 固定延迟等级。

证据：[ReservationTransactionalService.enqueueTimeout()](../src/main/java/com/citypass/service/impl/ReservationTransactionalService.java)、[ReliableTaskRepository.enqueueAt()](../src/main/java/com/citypass/reliable/ReliableTaskRepository.java)。

<a id="section-23"></a>

### 5.2 谁把任务变成 TIMEOUT 消息？

[ReliableTaskScheduler.relay()](../src/main/java/com/citypass/task/ReliableTaskScheduler.java) 默认按 1000ms 的 fixedDelay 扫描，批量上限为 50；为了防止一批后部任务排队时租约先过期，它每次只领取紧接着要执行的一条。`claimReady()` 使用任务级 `locked_by / lease_until / version`，不同实例可并行领取互相独立的任务。`execute()` 识别 `ORDER_TIMEOUT` 后调用：
...
| 调度依据 | `tb_reliable_task.next_retry_time = offer_expire_time` |
| RocketMQ 延迟等级 | 不设置；任务到期后发送普通消息 |
| 发送方式 | 普通同步 send，不是异步回调 |

`ORDER_TIMEOUT_DELAY_LEVEL` 已从常量中移除。这样 `reservation.offer-minutes` 改动后，不需要人工维护另一份 Broker 档位映射；同一个任务行就能表达每笔订单真实、可配置且受活动结束时间约束的执行时刻。

`execute()` 还会检查 `SendStatus.SEND_OK`，只有发送被 Broker 明确确认且 owner/version 租约仍匹配时才标记 DONE；异常按指数退避重试，达到上限进入 DEAD。发送后进程退出仍可能造成重复 TIMEOUT，所以消费者不能依赖只收到一次。
...
```java
transactionalService.expireAndPromote(orderId, offerMinutes);
```

它与主动取消共享内部释放算法，但公共入口和数据库条件已经拆开：

| 参数 / 效果 | 主动取消 | 系统超时 |
| --- | --- | --- |
| `expectedUserId` | 登录用户 ID | `null`，系统路径不按登录身份限制 |
| `waitlistTerminalStatus` | `CANCELLED` | `EXPIRED` |
| `requireExpired` | `false` | `true` |
| 订单 CAS | 本人且 status=1 时 1→4 | status=1 且 `offer_expire_time<=now` 时 1→4 |
| 旧订单请求事实 | `CANCELLED` | `EXPIRED` |
| 适用时的关联候补状态 | 写 CANCELLED | 写 EXPIRED |

**订单表仍统一写数字 4，不会把 status 改成字符串 EXPIRED。** 取消原因由持久化请求事实区分；旧订单来源为 WAITLIST 时，超时路径还会把关联候补记录写成 EXPIRED。
...
因此真实机制是：**订单事务按真实截止时间登记本地任务 → 到期后 Relay 发送普通 TIMEOUT 消息 → 消费者用到期复合 CAS 关闭；另有到期订单扫描作为补充入口**。不能只写“有个 Scheduler 每 15 分钟取消一次”。

<a id="section-26"></a>

### 5.5 提前消息为什么也不能误关单

`expireAndPromote()` 最终为关闭 UPDATE 追加：

```java
if (requireExpired) {
    close.le("offer_expire_time", now);
}
```

因此系统超时必须同时满足 `id`、`status=1` 和 `offer_expire_time<=now`。任务调度时间是第一层准确性，数据库复合 CAS 是第二层保护：即使消息因人工注入、时钟短暂偏差或其他原因提前到达，也不能把仍在资格期内的未支付订单关闭。

当关闭返回 SKIPPED，但观察到订单仍未支付且截止时间仍在未来时，`cancelTimeoutOrder()` 会抛异常，让 MQ 稍后重试，而不是把提前检查当作已处理完成。Docker 运行配置显式统一 JVM 与 MySQL 的 Asia/Shanghai 时区；多主机部署仍应配合系统时钟同步。

> **状态条件和时间条件承担不同职责。** `status=1` 防止已支付或已取消订单再次关闭，`offer_expire_time<=now` 防止尚未到期订单被提前关闭；缺一项都不能完整表达 TIMEOUT。
...
计算 expiresAt
    ↓
order.offer_expire_time = expiresAt
    ↓
插入订单，状态 UNPAID
    ↓
enqueueTimeout(orderId, expiresAt)  //expiresAt 是 min(现在 + offerMinutes, 活动结束时间),避免活动结束了都没超时的情况
    ↓
tb_reliable_task 新增：
task_type = ORDER_TIMEOUT
status = PENDING
next_retry_time = expiresAt

================ 时间还没到 ================

ReliableTaskScheduler 每隔约 1 秒扫描
    ↓
claimReady(...)
    ↓
只有 next_retry_time <= NOW() 才能领取
    ↓
ORDER_TIMEOUT 被领取为 RUNNING
    ↓
producer.sendOrderTimeout(orderId)
    ↓
RocketMQ 普通 TIMEOUT 消息
    ↓
Broker 接收成功
    ↓
可靠任务标记 DONE
...
cancelTimeoutOrder(orderId)
    ↓
transactionalService.expireAndPromote(...)
    ↓
releaseOrPromote(..., requireExpired = true)
    ↓
数据库再次检查：
订单仍然 UNPAID？
offer_expire_time <= NOW()？
    ↓
满足
    ↓
关闭订单
...
有一个经典的并发竞争问题：用户恰好在支付截止时间附近点了支付，同时 TIMEOUT 超时消息也到了。到底算“支付成功”还是“超时取消”？

Case 1A 对应的是：支付先抢到数据库更新权并提交。也就是用户虽然已经接近截止时间，但 PAY 的 UPDATE ... WHERE status = UNPAID 先成功，把订单从 UNPAID → PAID，之后 TIMEOUT 再来时发现订单已经不是 UNPAID，所以退出。这个场景可以理解成“用户在截止前及时完成了支付”。

Case 1B 对应的是：超时先抢到数据库更新权并提交。即使 PAY 请求更早发起、甚至更早 SELECT 到 UNPAID，但它真正执行 UPDATE 比较晚；这时 TIMEOUT 已经在到期后先把订单从 UNPAID → CANCELLED/EXPIRED，所以 PAY 后续再更新时，因为 status = UNPAID 条件已经不成立，只能失败。这个场景可以理解成“用户虽然开始支付了，但真正落库时资格已经被超时线程抢先关闭”。
...
| TIMEOUT | `OrderMQConsumer.init()` 中 TIMEOUT 分支 → `ReservationServiceImpl.cancelTimeoutOrder()` → `ReservationTransactionalService.expireAndPromote()` |
...
sequenceDiagram
    ...
    D-->>P: changed 为 1
    T->>D: UPDATE 1 到 4，并要求 offer_expire_time 已到
...
PAY 事务已经把 MySQL 请求事实推进为 PAID，外层 Service 只刷新 Redis 投影；TIMEOUT 的 `publishReleaseResult(SKIPPED)` 直接返回，不写 EXPIRED 投影，不回补库存，也不新建候补订单。

<a id="section-29"></a>

### 6.2 Case 1B：超时先迁移并提交，支付退出

反过来，为了单独观察状态竞争，可以设定 PAY 请求在截止前已取得数据、构造了时间参数，但执行更新被调度延迟；TIMEOUT 在资格到期后先抢到订单写入。
...
合法性还取决于用户、期限等条件。TIMEOUT 在截止前没有竞争资格；PAY 在截止后也没有竞争资格。没有任何事件满足条件时，可能没有赢家；发生数据库异常时，不应假装都会正常返回 0 行。所谓“Only One”，准确含义是：**同一订单离开未支付状态的合法竞争迁移，最多有一个最终提交生效**，不是每轮必然恰好一个成功。
...
主动取消通过 `cancelAndPromote()`、到期关闭通过 `expireAndPromote()`，最终都进入内部 `releaseOrPromote()`，目标也都是 4。这仍然需要并发保护，因为**状态写成同一个值不代表副作用天然幂等**。两条释放链先竞争同一个活动库存行，再由订单复合 CAS 决定本次是否有资格继续。
...
第一次 TIMEOUT 正常关闭订单、提交释放或补位后，进程仍可能在向 Broker 完成消费确认前退出，Broker 也可能因确认链路异常再次投递 `TIMEOUT 10001`。后置 Redis 投影失败会被 `projectRequest()` 捕获并记日志，本身不会要求 MQ 重试。
...
| DB stock+1 / 候补补位 | 不执行 |
| 恢复 Redis 的新任务 | 不登记 |
| 旧订单请求事实 / Redis 终态投影 | 不执行；保持已提交的 PAID |
| 是否要求 MQ 再试 | 正常返回，不要求重试 |

所以无需靠“用户支付后立刻把 Broker 中的消息删掉”保证正确性；本支付链也没有删除消息。**消息到达只提出一个处理请求，当前订单状态决定它还能不能生效。**

即使消息提前到达，复合 CAS 的到期条件也会拒绝关闭；`cancelTimeoutOrder()` 会对“仍未支付且尚未到期”的 SKIPPED 抛异常，要求 MQ 稍后重试。
...
> 这 5 个 Case 本质上都在验证 **订单状态迁移对副作用的约束能力**。Case 2 是 `CANCEL` 和 `TIMEOUT` 并发竞争同一个未支付订单，只有成功完成 `UNPAID → CANCELLED` 的一方才有资格继续释放库存或候补补位；Case 3 是重复 PAY，只有第一次能完成 `UNPAID → PAID`，后续请求因状态不匹配失败；Case 4 是 TIMEOUT 消息重复投递，第一次已经关闭订单后，后续重投会因订单不再是 `UNPAID` 而跳过，避免重复释放；Case 5 是订单已经 PAID 后迟到的 TIMEOUT，此时旧消息不能覆盖新状态，直接失效；Case 6 是重复 CANCEL，第一次释放资源后，第二次因订单已取消而不能再次释放。它们共同说明：**真正保证幂等的不是“重复写同一个状态”，而是只有本次成功完成状态迁移的操作，才有资格继续执行库存、候补、Redis 恢复等副作用。**
...
flowchart TD
    subgraph DBTX["MySQL 事务：releaseOrPromote"]
        A[读取订单并锁定活动库存行] --> B[复合 CAS：订单 1 变 4]
        B --> C{是否允许并选到补位对象}
        C -->|否| D[更新请求事实，DB stock 加 1，登记恢复任务]
        C -->|是| E[更新请求事实、候补与新订单，登记转移和超时任务]
        D --> F[COMMIT]
        E --> F
    end
    F --> G[外层 Service 刷新 Redis 请求投影]
...
| --- | --- | --- |
| `reservation:request:{orderId}` 的 CANCELLED / PAID | 外层 `ReservationServiceImpl` | 否；只是 MySQL 请求事实的短期投影 |
| stock、holders、claim 的资源状态 | 可靠任务扫描器调用 Lua | 否；用于实际名额数据 |

恢复脚本先查永久 `reservation:restore:{orderId}` 标记；已存在就跳过。本次首次恢复时，stock Key 必须存在，而且 claim 必须匹配 `orderId:resourceVersion`，脚本才会清理 holder 与 claim、回补一次库存并写标记。v3 无版本直接预约只在期望版本为 0 时兼容。

特别注意：**归属版本防止旧任务释放已经交给下一人的名额，永久 marker 防止同一恢复任务再次加库存**。两者承担不同职责；活动归档后若要清理永久 marker，需要另行制定离线保留策略，不能随意给短 TTL。
...
### 9.4 成功事务后的 Redis 投影失败，不该再取消一遍

例如关闭事务已提交时，订单状态、请求事实和资源任务已经一起落库。随后 `publishReleaseResult()` 刷新 Redis 投影即使失败，也只记录警告，不会让 MQ 消费失败或重新取消。支付路径同理：请求事实 `PAID` 已在支付事务内提交。

`getReservationResult()` 先读取 `tb_reservation_request` 校验归属，再组合订单或候补结果。因此 Redis 投影暂时缺失不会影响 v4 请求的正确查询；它以后被其他业务动作刷新只是性能优化，不是恢复正确性的必要步骤。
...
因此调试时看到返回对象仍带旧 status，不能立刻断定数据库 CAS 没执行。`publishReleaseResult()` 也不是从这个对象的旧 status 推断关闭状态，而是按成功动作和调用方传入的终态刷新 Redis 投影；MySQL 请求事实已经在事务中写好。
...
| 1 未支付，期限内 | TIMEOUT | 到期复合 CAS 不命中；若仍在未来则抛异常要求 MQ 重试 | 不改状态，不提前释放 |
| 1 未支付，期限已到 | TIMEOUT | status 与 `offer_expire_time<=now` 同时命中 | 1→4，释放或补位一次，请求事实写 EXPIRED |
...
下面省略其他候补，只看订单 10001。时间是示意，可靠任务按 10:15 到期领取，但扫描、发送和消费可能让 TIMEOUT 晚于 10:15 到达。
...
| 10:08:00.120 | 支付事务提交 | 成功 | 已提交 2，请求事实 PAID | 不释放，随后尽力刷新 Redis 投影 |
...
| --- | --- | --- | --- | --- | --- |
| T0 | 初始 | 1 | 9 | stock=9，Claim 属于 `10001:0` | 未支付，请求事实 RESERVED |
| T1 | 到期后 TIMEOUT 关闭事务执行 | 事务内 4 | 事务内 10 | 尚未由关闭事务修改 | 复合 CAS 命中，请求事实 EXPIRED，登记恢复任务 |
| T2 | 关闭事务提交 | 4 | 10 | 可能仍为 stock=9、旧 Claim | 关闭业务与请求结果成立 |
| T3 | 可靠任务正常执行 | 4 | 10 | stock=10，按版本清理旧归属，写永久恢复标记 | Redis 资源推进完成 |
...
| `releaseOrPromote()` | 活动库存行 `FOR UPDATE` + 订单条件 UPDATE | 串行化同活动释放/入队，并决定当前关闭事件能否拿到释放权 |
| `ReservationWaitlistMapper.selectNextForUpdate()` | 阻塞式 `FOR UPDATE` | 锁定真正队首，不用 SKIP LOCKED 跳过被占用的较早候补 |
| `ReliableTaskScheduler.relay()` | 任务行 `FOR UPDATE SKIP LOCKED` + owner/lease/version | 多实例并行领取互相独立任务，并回收过期租约 |
| `ReservationReconcileTask.reconcile()` | Redisson 分布式扫描锁 | 避免多个实例同时执行全局对账扫描 |
...
| 3 | [src/main/java/com/citypass/service/impl/ReservationServiceImpl.java](../src/main/java/com/citypass/service/impl/ReservationServiceImpl.java) · `payOrder()` | 登录与参数检查，调用事务，成功后刷新 Redis PAID 投影 |
| 4 | [src/main/java/com/citypass/service/impl/ReservationTransactionalService.java](../src/main/java/com/citypass/service/impl/ReservationTransactionalService.java) · `payUnpaidOrder()` | 读取归属，复合 CAS 1→2，同事务更新请求事实及关联候补 |
...
| 6 | [src/main/java/com/citypass/service/impl/ReservationServiceImpl.java](../src/main/java/com/citypass/service/impl/ReservationServiceImpl.java) · `projectRequest()` | 事务后尽力更新短期 Redis 投影 |
...
| 2 | [src/main/java/com/citypass/service/impl/ReservationServiceImpl.java](../src/main/java/com/citypass/service/impl/ReservationServiceImpl.java) · `cancelOrder()` | 传入登录用户并调用主动取消入口 |
| 3 | [src/main/java/com/citypass/service/impl/ReservationTransactionalService.java](../src/main/java/com/citypass/service/impl/ReservationTransactionalService.java) · `cancelAndPromote()` / `releaseOrPromote()` | 锁活动资源、本人 + status CAS、请求事实、资源变化及任务登记的事务 |
...
| 7 | [src/main/java/com/citypass/service/impl/ReservationServiceImpl.java](../src/main/java/com/citypass/service/impl/ReservationServiceImpl.java) · `publishReleaseResult()` | 成功事务后刷新 Redis 请求投影，非 MQ 发布 |
...
| 顺序 | 路径与方法 | 负责什么 |
| --- | --- | --- |
| 1 | [src/main/java/com/citypass/service/impl/ReservationTransactionalService.java](../src/main/java/com/citypass/service/impl/ReservationTransactionalService.java) · `createOrder()` / `enqueueTimeout()` | 建单事务以 `offerExpireTime` 调用 `enqueueAt()` |
| 2 | [src/main/java/com/citypass/reliable/ReliableTaskRepository.java](../src/main/java/com/citypass/reliable/ReliableTaskRepository.java) · `enqueueAt()` / `claimReady()` | 保存定时 PENDING 任务，以租约和版本领取到期任务 |
| 3 | [src/main/java/com/citypass/task/ReliableTaskScheduler.java](../src/main/java/com/citypass/task/ReliableTaskScheduler.java) · `relay()` / `execute()` | 到期后取任务，调用发布接口并检查 SEND_OK |
| 4 | [src/main/java/com/citypass/mq/OrderMessagePublisher.java](../src/main/java/com/citypass/mq/OrderMessagePublisher.java) / [RocketMQProducer.java](../src/main/java/com/citypass/mq/RocketMQProducer.java) · `sendOrderTimeout()` | 发送只含订单 ID 的普通 TIMEOUT 消息 |
| 5 | [src/main/java/com/citypass/utils/RocketMQConstants.java](../src/main/java/com/citypass/utils/RocketMQConstants.java) | Topic 与 TIMEOUT Tag；不再维护固定延迟等级 |
...
| 7 | [src/main/java/com/citypass/service/impl/ReservationServiceImpl.java](../src/main/java/com/citypass/service/impl/ReservationServiceImpl.java) · `cancelTimeoutOrder()` | 调用到期入口，并让提前消息抛异常重试 |
| 8 | [src/main/java/com/citypass/service/impl/ReservationTransactionalService.java](../src/main/java/com/citypass/service/impl/ReservationTransactionalService.java) · `expireAndPromote()` / `releaseOrPromote()` | 使用 status + 到期复合 CAS，与取消共享 winner / loser 资源路径 |
...
| 文件链接 | 用途 |
| --- | --- |
| [src/main/java/com/citypass/entity/ReservationOrder.java](../src/main/java/com/citypass/entity/ReservationOrder.java) | 状态字段、来源、支付截止时间、资源版本 |
| [src/main/java/com/citypass/entity/ReservationRequest.java](../src/main/java/com/citypass/entity/ReservationRequest.java) | 持久化请求状态、归属、orderId 与最后错误 |
...
> 我把它称为 CAS 式条件迁移，例如支付只有在订单属于当前用户、状态仍是未支付且支付时间条件满足时，才改成已支付。主动取消要求本人且旧状态为未支付；系统超时要求旧状态为未支付并且资格已到期，两者都改成已取消。影响行数决定本次是否成功，失败就返回 false 或 SKIPPED。底层仍有 InnoDB 写锁协调，不是无锁算法，也没有版本号自旋。
...
> PAY 和 TIMEOUT 最终都更新同一订单，并要求旧状态仍为未支付。支付还要求本人和 `offer_expire_time` 晚于检查时间，超时则要求 `offer_expire_time` 已到。支付事务先迁移提交，超时会前置跳过或条件更新零行，不释放资源；超时事务先提交，支付也不再匹配。最终以事务提交为准，先 UPDATE 但后续回滚不算最终胜出；即使 TIMEOUT 消息提前到达，到期复合 CAS 也不会误关仍有效的订单。

<a id="section-67"></a>

### 🔥 重点 5：CANCEL 和 TIMEOUT 为什么不重复释放库存？

**详细理解**：这两个操作的目标都是取消，但主动取消与系统到期的准入条件不同，资源恢复仍然不能各做一次。`cancelled==0` 后的 return 是关键，它把 loser 挡在库存、候补和任务写入之前。

**面试回答**：

> 主动取消和超时分别从 cancelAndPromote、expireAndPromote 进入，再复用内部 releaseOrPromote。主动取消校验用户，超时追加到期条件；只有条件更新成功的事务才进入库存恢复或候补补位，另一方返回 SKIPPED。资源动作、请求事实与状态迁移在同一数据库事务里，所以成功者后续出错会整体回滚。重复执行已经取消的订单会先被状态查询跳过，避免再次加库存或生成补位订单。
...
> TIMEOUT 即使重复到达，第一次关闭提交后订单已经不是未支付，后续消费会 SKIPPED，不再重复释放。若两个消费并发都先读到未支付，则由后面的状态与到期复合 UPDATE 决定谁继续。如果第一次数据库事务回滚，重试重新关闭是正确恢复，不属于重复生效。请求终态由 MySQL 请求事实保存；已登记 Redis 资源任务的重复执行还由任务业务键、资源版本和永久 Lua 标记处理，不能只靠订单状态 CAS。
...
| “publishReleaseResult 会发送取消 MQ。” | 此方法只刷新 Redis 请求投影并记录日志，不发送 MQ；MySQL 请求事实已在事务内更新 |
| “数据库事务一起回滚 Redis Claim。” | Redis 资源由独立可靠任务处理，不在这笔事务里 |
| “TIMEOUT 到达就可以直接把未支付订单关闭。” | TIMEOUT 任务虽按截止时间调度，关闭 UPDATE 仍复核 `offer_expire_time<=now`；提前消息会被拒绝并重试 |
...
| --- | --- | --- |
| 生命周期状态机 | 数字 status=1/2/4，`createOrder()`、`payUnpaidOrder()`、`cancelAndPromote()`、`expireAndPromote()` | 基于字段与条件更新，没有专门状态机框架；3/5/6 暂只有定义 |
| CAS 条件化状态迁移 | 支付和关闭 UpdateWrapper 的 `.eq("status", ORDER_STATUS_UNPAID)` | 支付另有用户、期限条件；TIMEOUT 另有到期条件，主动取消另有用户条件 |
| 幂等校验 | 关闭前置状态判断；重复支付被条件拒绝；请求事实、资源版本和可靠任务独立去重 | 重复 HTTP 操作不一定返回成功；整体语义是至少一次 + 业务幂等，不声称跨系统 exactly-once |
...
>
> 主动取消和超时分别从 cancelAndPromote、expireAndPromote 进入，再共用内部 releaseOrPromote。两者都要求旧状态为 1，主动取消追加本人条件，超时追加截止时间已到条件。影响零行就 SKIPPED，不再触碰库存或候补；成功者才处理释放或补位，并把订单状态、请求事实、数据库资源变化和可靠任务一起提交。所以支付先提交时，超时不能再关闭；超时先提交时，支付也不能再成功。重复关闭串行到达时，通常直接在前置查询处跳过。
>
> TIMEOUT 来自建单事务按真实 `offerExpireTime` 登记的任务。Relay 到期后以 owner、lease 和 version 领取任务，发送普通 RocketMQ 消息并检查 SEND_OK；另有到期订单扫描兜底。支付成功不撤回消息，而是在消费时按当前状态和到期条件判断是否还应处理。Redis 资源操作在数据库事务外通过可靠任务执行，不能只靠订单 CAS 保证跨存储完成。
>
> 当前实现也有明确边界：重复支付返回业务失败，只保证本地状态不重复；支付目前只是项目内状态演示，没有接入真实渠道的签名、回调幂等和账务对账。可靠任务保证可重试并进入 DEAD，也仍需监控告警和人工重放，不能宣传为跨系统 exactly-once。
...
当前三条状态链没有共同的 Redis 订单锁，数据库条件已经能表达“一张未支付订单只能被一个迁移消耗”。如果额外加锁，需要让各入口都遵守同一锁协议，并处理失效与异常，但最终数据库条件仍值得保留。项目中 CREATE 用户锁、对账扫描锁以及可靠任务租约解决的是其他竞争，不应混为一谈。
...
当前恢复标记不设 TTL，以空间换取长期重试幂等；活动归档后需要配套离线清理策略。任务发送与执行仍可能至少一次发生，完整恢复保证要单独验证，不能只凭有任务表就称为跨系统 exactly-once（恰好一次效果）。
...
当前超时专用到期条件、请求事实和可靠任务都已经补齐。接下来最值得补的是**真实支付渠道边界**：现在的支付接口只把本地订单从 1 改为 2，没有支付单、渠道流水唯一约束、签名验真、异步回调幂等、退款与对账。面试时可以把现有代码准确称为“支付状态竞争演示”，若继续升级，再围绕渠道流水号和回调事实设计独立支付域，不能把一次本地 UPDATE 说成完整支付系统。
...
| CANCEL 链 | Controller.cancel → Service.cancelOrder → Transactional.cancelAndPromote → 内部 releaseOrPromote |
| TIMEOUT 来源 | 建单事务任务 → relay → sendOrderTimeout → Consumer；另有到期扫描 |
| Topic / Tag / 调度 | `citypass-reservation-topic` / `TIMEOUT`；任务按真实截止时间到期，随后发送普通消息 |
| 支付 CAS | ID、用户、status=1、截止时间条件；`changed!=1` 退出 |
| 关闭 CAS | 主动取消：ID、本人、status=1；系统超时：ID、status=1、`offer_expire_time<=now`；`cancelled==0` 返回 SKIPPED |
...
| 锁 | 活动库存行串行化释放与候补入队；订单条件 UPDATE 决定终态；任务行租约只协调 Relay |

CREATE 与 TIMEOUT 都由同一个 Relay 调用同步 `send()`，并统一检查 `SendStatus.SEND_OK`；只有 Broker 明确确认、且任务的 owner/version 租约仍匹配时才改为 DONE。失败按指数退避，达到 `max_retry` 进入 DEAD，可通过受令牌保护的管理接口显式重放。发送确认后宕机仍可能造成重复投递，因此这套机制的口径仍是至少一次 + 业务幂等。
...
| [scripts/reliability-test.ps1](../scripts/reliability-test.ps1) | 真实 MySQL/Redis/RocketMQ 下验证支付与超时边界仅一个终态、双释放只补位两人、锁住队首不跳号、Lua 重试只回补一次 | 验证的是受控功能与并发场景，不是生产流量、网络分区或真实支付渠道测试 |
...
2026-09-09 已在全新 Docker 数据卷运行 `mvn clean test`、`smoke-test.ps1` 与 `reliability-test.ps1`。其中可靠性脚本实测 100 用户争抢 10 份库存无超卖、Broker 停机后请求恢复、锁住真正候补队首时不跳号、支付/超时边界只有一个终态，以及库存恢复 Lua 连续执行两次只增加一次。它们是功能与并发正确性验收，不是 QPS 压测，也没有接入真实支付渠道。

**格式检查**：27 个不同的相对源码链接均按仓库 `docs/` 位置核实存在；完整目录覆盖 80 个章节和小节。8 张 Mermaid 图均通过 Mermaid CLI 11.17.0 解析，代码围栏配对完整。未在真实 GitHub 页面做渲染预览。
