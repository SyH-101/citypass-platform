...
/**
 * 业务层滑动窗口限流：限量预约接口按 userId 限流（需在登录拦截器之后）
 */
@Slf4j
@Component
public class SlidingWindowInterceptor implements HandlerInterceptor {
    ...
    @Value("${reservation.rate-limit.sliding-window.enabled:true}")
    private boolean enabled;

    @Value("${reservation.rate-limit.sliding-window.window-ms:1000}")
    private long windowMs;

    @Value("${reservation.rate-limit.sliding-window.max-requests:5}")
