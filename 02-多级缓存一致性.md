...
public Result update(Venue venue) {
    ...
    reliableTaskRepository.enqueue(
            ...
            JSONUtil.toJsonStr(
                    new VenueCacheInvalidation(
                            id,
                            refreshed.getCacheVersion()
                    )
            )
    );
 ...
 * 1. 更新 Venue 业务字段；
 * 2. 推进 cacheVersion；
 * 3. 记录“必须失效 Venue 缓存”的 ReliableTask；
 * 4. 推进受影响搜索文档的版本，并登记搜索索引任务。
 *
 * 这样做的核心目的，是把“数据库事实变化”和“后续必须发生的异步副作用”
 * 可靠地绑定在同一个本地事务里。
 */
@Transactional
public Result update(Venue venue) {

    // 从前端传入的 Venue 对象中取出主键。
    // 后续更新数据库、推进版本以及生成缓存失效任务，都需要这个 id。
    Long id = venue.getId();

    // 更新 Venue 必须知道具体是哪一条记录。
    // 如果连 id 都没有，就没有继续执行的意义，直接返回业务失败。
    if (id == null) return Result.fail("场馆id不能为空");

    // 在真正修改 Venue 之前，先检查当前搜索索引治理状态是否允许业务写入。
    //
    // 原因：
    // Venue 的 name、address、area、coordinates 等字段会被冗余到 Activity 搜索文档中。
    // 如果当前系统正在进行搜索索引全量重建，而业务侧又同时修改 Venue，
    // 可能导致“全量重建旧数据”和“在线写入新数据”发生交错。
    //
    // 因此这里先做写入保护。
    searchRebuildStateRepository.assertWritesAllowed();
    ...
    // 真正更新 Venue 的业务字段。
    //
    // updateById(venue) 最终会根据 venue.id 更新数据库中的对应记录。
    // 例如：
    //
    // Venue 2：
    // name = "城市音乐空间"
    ...
    //
    // 注意：
    // 这里主要更新业务字段，cacheVersion 会在下一步单独推进。
    if (!updateById(venue)) {

        // 如果数据库中不存在这条 Venue，
        // 或者更新没有成功影响目标记录，则直接返回失败。
        return Result.fail("场馆不存在或更新失败");
    }

    // 单独推进 Venue 的 cacheVersion。
    //
    // 例如：
    //
    // 更新前：
    ...
    // cacheVersion = 5
    //
    // cacheVersion 不是“修改次数统计”，而是缓存一致性中的“数据代次”。
    //
    // 后续 Redis / OpenResty 会通过版本水位判断：
    //
    // v4 < watermark 5
    // → 说明 v4 已经过时
    // → 不允许旧版本重新回填缓存
    //
    // incrementCacheVersion(id) 正常情况下应该只更新 1 行。
    if (getBaseMapper().incrementCacheVersion(id) != 1) {
    ...
    // 为什么不能直接继续使用参数 venue？
    //
    // 因为 cacheVersion 是通过上一条独立 SQL 在数据库中递增的，
    // 传进来的 venue 对象本身不一定已经同步成最新版本。
    //
    // 因此需要重新查询一次，得到：
    //
    // Venue 2
    // cacheVersion = 5
    //
    // 下面的缓存失效任务必须携带这个最新 committed version。
    Venue refreshed = getById(id);

    // 将“Venue 缓存必须失效”这件事登记为 ReliableTask。
    //
    // 注意：
    // 这里并没有直接 redis.delete(...)。
    //
    // 当前设计采用的是本地 Outbox / ReliableTask 思路：
    //
    // Venue 更新
    // + cacheVersion++
    // + INVALIDATE_VENUE_CACHE 任务
    //
    // 一起写入 MySQL 事务。
    //
    // 这样即使事务 COMMIT 后服务立刻宕机，
    // “后续必须失效缓存”这个意图仍然已经持久化，不会凭空丢失。
    reliableTaskRepository.enqueue(

            // 任务类型：
            // 告诉 ReliableTaskScheduler，
            // 这是一条 Venue 缓存失效任务。
            ReliableTaskRepository.INVALIDATE_VENUE_CACHE,
            ...
            // 则 bizKey 类似：
            //
            // invalidate-venue-cache:2:5
            //
            // 它表达的是：
            // “Venue 2 的第 5 版对应的缓存失效任务”。
            "invalidate-venue-cache:" + id + ":" + refreshed.getCacheVersion(),
            ...
            // DEL cache:venue:2
            //
            // 还需要把 Redis / OpenResty 的版本水位推进到至少这个版本。
            //
            // 例如：
            //
            // venueId = 2
            ...
            // 后续可执行：
            //
            // watermark = max(currentWatermark, 5)
            //
            // 从而阻止 v4 之类的迟到旧结果重新生效。
            JSONUtil.toJsonStr(
                    new VenueCacheInvalidation(
                            id,
                            refreshed.getCacheVersion()
    ...
    // Venue 的部分字段还会被冗余进 Activity 的搜索文档。
    //
    // 因此 Venue 更新后，不仅缓存副本要收敛，
    // 搜索索引副本也必须最终更新。
    //
    // 这一步会：
    //
    // 1. 推进受影响 Activity 搜索文档的 searchVersion；
    // 2. 登记对应的搜索索引任务。
    //
    // 所以一次 Venue 更新实际上会同时产生两条“派生副本更新链”：
    //
    // Venue 事实更新
    // ├─ 缓存副本收敛
    // └─ 搜索索引副本收敛
    ...
    // + 搜索索引任务落库
    //
    // 已经在当前 MySQL 本地事务中顺利执行。
    //
    // 它并不表示：
    //
    // Redis 已经删除完毕
    // Caffeine 已经全部清理
    // OpenResty 已经 purge 完毕
    // Elasticsearch 已经更新完毕
    //
    // 这些动作会在事务提交后由后台异步链路继续完成。
    return Result.ok();
}
```
...
```text
ReliableTaskScheduler
        ↓
INVALIDATE_VENUE_CACHE
        ↓
推进 Redis version watermark
        ↓
删除 Redis 缓存
        ↓
Pub/Sub 清理在线 JVM 的 Caffeine
        ↓
版本化 Purge OpenResty
```
...
令牌桶（Token Bucket）是一种常见的限流算法，可以把它想象成一个会自动补充“通行证”的桶。桶里最多只能放固定数量的令牌，请求到来时必须先拿走 1 个令牌，拿得到就放行，拿不到就拒绝。与此同时，系统会按照固定速度持续往桶里补充令牌，因此它既能限制长期平均请求速率，又允许短时间内出现一定突发流量。

以当前 CityPass 的 Venue 查询为例，OpenResty 对 `venue-read:{客户端 IP}` 使用独立令牌桶，默认按 **60 个令牌/秒** 的速度补充，桶容量为 **120**。假设桶一开始有 120 个令牌，那么短时间内突然来了 100 个请求，这 100 个请求都可以立即拿到令牌并通过；但如果请求持续高速涌入，令牌会逐渐被消耗。当桶里已经没有令牌，而新的令牌又还没来得及补充时，后续请求就会被限流并返回 `429 Too Many Requests`。因此，“60 次/秒”控制长期速率，“120”则允许一定的瞬时突发。
...
CityPass 在这里使用令牌桶，主要是为了防止攻击者不断请求不同的随机 Venue ID。`null marker` 只能挡住“同一个不存在 ID 被重复查询”，但如果攻击者不断换成新的随机 ID，那么每个新 ID 第一次都可能穿透 Redis 并继续向后查询。OpenResty 的按 IP 令牌桶会在更靠近入口的位置限制这种扫描流量，避免大量随机请求持续进入 Java、Redis 和 MySQL。当前 `token_bucket.lua` 还通过 `ngx.shared` 保存桶状态，并使用短锁保证多个 OpenResty worker 并发更新同一个桶时不会把令牌重复发放；如果抢锁或脚本执行异常，则采取保守拒绝策略，避免限流机制失效后放大流量。

> 可以简单记忆：**null marker 解决“重复查同一个不存在 ID”，令牌桶解决“不断换随机 ID 进行扫描”。**
