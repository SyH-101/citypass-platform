...
> **最终一致性与缓存治理：基于本地可靠任务表实现类 Transactional Outbox，使用任务级租约、执行版本、重试与 DEAD 隔离处理外部副作用；结合 Reconciliation 修复部分业务漂移，并以缓存失效 Outbox、版本水位、Redis Pub/Sub、watchdog 互斥及受控 DB 降级治理 OpenResty + Caffeine + Redis 多级缓存。**
...
源码基线起点：Git 提交 `7b51ffafcea428e2877db0541709d130828236e7`；网关一致性与缓存可靠性增强按 2026-09-19 当前源码复核。本文按实际 Java、SQL、Lua、配置和部署文件写作，注释与执行逻辑冲突时以执行逻辑为准；所有相对链接均从当前仓库根目录文档定位。
...
- [3. ReliableTask 表中到底保存了什么？](#section-13)
  ...
  - [3.3 七种真实 task_type 与 payload](#section-16)
...
- [12. 从全 miss 到命中：每层回填了什么？](#section-48)
  ...
  - [12.5 为什么版本和正文必须是一个原子缓存项？](#section-52a)
  - [12.6 purge 与慢响应回填怎样避免交错？](#section-52b)
  - [12.7 读侧为什么仍要比较版本水位？](#section-52c)
  - [12.8 哪些请求和响应明确不进入网关缓存？](#section-52d)
  - [12.9 网关命中与 Java 直连的鉴权语义一致吗？](#section-52e)
  - [12.10 多个网关实例怎样逐一确认失效？](#section-52f)
  - [12.11 怎样观察和验证网关缓存决策？](#section-52g)
...
- [17. 八种缓存故障窗口，逐个推演怎样恢复](#section-72)
  ...
  - [17.7 Case 7：Redis 整体不可用](#section-78a)
  - [17.8 Case 8：锁等待耗尽](#section-78b)
...
| Versioned Invalidation | Redis 与 OpenResty 各自推进版本水位；正常回填比较版本 | 在线 JVM 用 Pub/Sub 驱逐；静态网关拓扑、故障降级和新增路径仍有边界 |
...
| `INDEX_ACTIVITY_SEARCH` | 活动创建、搜索字段/状态或关联 Venue 变化，`index-activity-search:{活动号}:{searchVersion}` | activityId 与 requestedVersion | 执行时回读 MySQL 最新事实，以 ES external_gte 版本写活动文档 |
...
普通建单在同一事务创建订单、把请求事实推进为 RESERVED，并用 `enqueueAt` 把 ORDER_TIMEOUT 的 `next_retry_time` 直接设为 offerExpireTime。补位事务还会写归属迁移任务；限量活动发布会写 INIT_RESERVATION_STOCK，普通与限量活动发布都会写 INDEX_ACTIVITY_SEARCH。

Venue 更新也已纳入任务表：更新数据库、递增 cacheVersion、写 INVALIDATE_VENUE_CACHE，同时推进关联活动的 searchVersion 并逐项写 INDEX_ACTIVITY_SEARCH；这些变化同事务提交。它不再依赖提交后内存回调保存失效意图。
...
[ReliableTaskScheduler.relay](./src/main/java/com/citypass/task/ReliableTaskScheduler.java) 默认以 `fixedDelay=1000ms` 调度。每轮最多处理 batchSize=50，但每次只调用 `claimReady(1, instanceId, leaseSeconds, includeSearchTasks)` 领取即将执行的一条，执行完成后才领取下一条，避免批尾任务还在内存排队时 30 秒租约已经过期。搜索模块关闭时，领取 SQL 排除 INDEX_ACTIVITY_SEARCH，相关任务继续保持 PENDING；其他六类任务照常执行。
...
经仓库 OpenResty 部署访问无 query string 的 `GET /venues/1` 时，入口先经过独立的 Venue 详情 IP 令牌桶，再检查网关共享字典。未命中才进入 [VenueController.queryVenueById](./src/main/java/com/citypass/controller/VenueController.java)，再调用 [VenueServiceImpl.queryById](./src/main/java/com/citypass/service/impl/VenueServiceImpl.java)。
...
```mermaid
flowchart TD
    U["GET /venues/1"] --> RL{"IP 读令牌桶放行？"}
    RL -->|否| X["429"]
    RL -->|是| G{"OpenResty 命中？"}
    G -->|版本不低于水位| A["返回同一缓存项中的 version + body"]
    G -->|旧版本或 miss| J["VenueController → VenueServiceImpl"]
    ...
    R -->|Redis ERROR| H["failure gate + DB fallback bulkhead"]
    H -->|有 permit| I["少量请求查 DB，可写 Caffeine"]
    H -->|无 permit| K["503 快速失败"]
```
...
|---|---|---|---|
| OpenResty | `/venues/{id}` 的 `cacheVersion + HTTP 正文` 原子项 | 命中时不进入 Java，读取时比较本实例版本水位 | 当前网关实例共享字典 |
...
OpenResty、Caffeine 与 Redis 都 miss 后，Java 用 Redisson 获取 `lock:venue:rebuild:{id}`，再检查一次 Redis；仍不存在才 `getById`。调用 `RLock.tryLock()` 时不传固定 lease，持锁线程存活期间由 watchdog 自动续租。数据库返回的 Venue 带 cacheVersion。

回填不再是普通 SET。`write-cache-if-version.lua` 比较数据库行 version 与 `cache:venue:version:{id}` 水位：只有 expectedVersion≥currentVersion 才写 RedisData 与物理 TTL。冷加载只有在脚本接受写入后才装入当前 JVM 的 LocalEntry；成功响应由 Controller 同时提供 `X-Venue-Cache-Version` 与内部可信标记，OpenResty 再把该 version 和完整正文编码成一个缓存值。
...
| Redis 异步重建查不到数据 | 写 2 分钟 null marker，并驱逐当前本地项 |
...
不存在的 ID 若每次查库会形成缓存穿透。当前代码用带 `nullValue=true`、version 与 expireTime 的 RedisData JSON 表示已查证不存在；空白字符串会被 `readRedis` 当作 miss，不再是负缓存格式。同一 ID 第二次查询因此不会再次访问 MySQL。

Caffeine 不缓存 null。Java 的业务失败通常仍是 HTTP 200，但 `VenueController` 只对成功且 data 为 Venue、cacheVersion 合法的响应发送 `X-Gateway-Cacheable: 1`。OpenResty 要求这个内部标记，因此“场馆不存在”不会进入网关缓存。

null marker 只解决**重复的不存在 key**。攻击者连续换 ID 仍会制造首次 miss，所以精确详情 GET 还在 OpenResty 复用现有 `token_bucket.lua`，按 IP 使用独立共享字典和桶状态；默认每秒 60、突发 120，超量返回 429。它与预约入口限流分开，读阈值也远宽于业务层每用户每秒 5 次的预约限制。

本次没有增加 Bloom Filter。要保证 Bloom 没有 false negative，必须在启动时完整装载已有 Venue，并让所有新增、删除路径可靠同步；而当前 `POST /venues` 尚未进入统一版本化写链。此时加入 Bloom 会新增一套容易漏同步的正确性状态。对当前实习项目，网关随机 key 限流加 Redis null marker 的收益和复杂度更合适；它仍不是完整 WAF 或 DDoS 防护。
...
见 [VenueController](./src/main/java/com/citypass/controller/VenueController.java) 与 [nginx.conf](./src/main/resources/openresty/nginx.conf)：Java 只为成功返回的真实 Venue 附加版本头和内部可缓存标记。OpenResty 还要求状态为 200、版本是非负整数、Content-Type 为 JSON、没有 Set-Cookie、Cache-Control 不含 private/no-store，且完整正文不超过 1 MiB。

`X-Gateway-Cacheable` 是 Java 到网关的信任信号，header filter 会在返回客户端前删除。网关不需要解析 `Result` JSON，也不会因普通业务失败同为 HTTP 200 而误缓存。

<a id="section-52a"></a>

### 12.5 为什么版本和正文必须是一个原子缓存项？

OpenResty 使用 `版本文本 + 换行符 + 原始正文` 编码一个 shared dict value。读取时先拆出 version，再返回原始 body。没有把 version 和 body 分成两个共享键，因此读请求不会看到“新 version 配旧 body”或“旧 version 配新 body”的中间组合。

版本水位单独保存在 `venue_cache_version` 字典，因为它表达的是这个网关实例对该 venueId 的**最低可接受代际**，生命周期不同于可被 TTL/LRU 淘汰的响应正文。写水位使用 `safe_set`；容量不足会让 purge 或回填显式失败，不用 LRU 静默丢掉正确性元数据。

<a id="section-52b"></a>

### 12.6 purge 与慢响应回填怎样避免交错？

`venue_cache.lua` 对同一 venueId 使用 `resty.lock` 短锁。purge 在锁内执行 `max(当前水位, committedVersion)`，并删除低于新水位的缓存项；body filter 回填也在同一把锁内重新读取水位，只有 `responseVersion >= watermark` 才写入。body filter 不能 yield，因此锁忙时直接放弃本次可选回填，让响应正常返回。

关键时间线如下：

```mermaid
sequenceDiagram
    participant R as 旧 GET
    participant J as Java
    participant P as v5 purge
    participant G as OpenResty
    R->>J: miss 后开始查询 v4
    P->>G: DELETE id=1&version=5
    G->>G: venue 锁内 watermark=max(old,5)，删除旧项
    J-->>G: 迟到响应 X-Venue-Cache-Version: 4
    G->>G: venue 锁内比较 4 < 5
    G-->>R: 响应可返回，但拒绝写入 shared dict
```

因此“先检查水位，再无锁写正文”的 check-then-act 竞态不存在。若回填先拿锁并写 v4，后到的 v5 purge 会删掉它；若 purge 先完成，v4 回填会被拒绝。两种线性化顺序都不会让 v4 在 v5 purge 完成后复活。

<a id="section-52c"></a>

### 12.7 读侧为什么仍要比较版本水位？

即使正常 purge 会删除旧项，读取仍在同一场馆锁内同时取缓存项与水位；若发现 `entryVersion < watermark`，立即删除并按 MISS 回源。这层检查覆盖异常重载、未来代码改动或历史残留导致“旧项与新水位并存”的防御场景。

读取锁定义了并发顺序：若 HIT 先获得锁，它在线性化意义上发生在 purge 之前；若 purge 已经完成，之后的读取一定看到更高水位并拒绝旧项。项目没有声称能撤回 purge 前已经开始向客户端发送的旧响应，但保证 purge 完成后的新缓存判定不接受低版本。

<a id="section-52d"></a>

### 12.8 哪些请求和响应明确不进入网关缓存？

缓存与 IP 读限流范围都是**无 query string 的精确 `GET /venues/{id}`**。非 GET、带查询参数的详情请求、列表/基线接口、业务失败、私有响应、带 Cookie 的响应、非 JSON、无合法版本头和超过 1 MiB 的正文都只代理、不回填。当前详情结果只依赖 id，但显式绕过 query string 可以防止未来加入 `preview`、语言或字段选择参数后错误复用旧 key。

TTL 仍是 540～660 秒，body filter 用 chunk table 累计正文，超过上限立即停止收集。版本协议解决代际倒退，TTL 仍负责容量和失效兜底，两者职责不同。

<a id="section-52e"></a>

### 12.9 网关命中与 Java 直连的鉴权语义一致吗？

`GET /venues/{数字 id}` 被明确列为公共读，8080 命中和 8081 直连都不要求登录；Venue 的分类、名称与基线读接口也按精确规则公开。PUT/POST 等写请求不满足 GET 条件，仍由 LoginInterceptor 拒绝匿名访问。原来宽泛的 `/venues/` 前缀已收紧，未知子路径不会因为前缀相同自动公开。

边界是 token 滑动续期：网关 HIT 不进入 `RefreshTokenInterceptor`，所以即使请求带 token，也不会靠这次公共详情读取刷新 Redis 登录 TTL。这不改变“能否读取公共 Venue”的授权结果，但客户端不能依赖网关缓存命中为登录态续期。

<a id="section-52f"></a>

### 12.10 多个网关实例怎样逐一确认失效？

`gateway-cache.purge-urls` / `GATEWAY_CACHE_PURGE_URLS` 接受逗号分隔的明确地址；旧的 `purge-url` / `GATEWAY_CACHE_PURGE_URL` 继续兼容，二者合并去重。Invalidator 把 Redis Lua 返回的实际水位作为 `version`，逐一发送 `DELETE ?id=...&version=...`，而不是只通知一个不带版本的地址。

所有地址成功后 handler 才返回。任一节点失败会记录 endpoint、venueId 与 version，并在尝试完其余节点后抛异常；ReliableTask 因此不会标 DONE，而会进入既有退避重试或 DEAD 流程。当前仍是静态显式地址，不包含服务发现、动态拓扑或逐节点持久确认表。

<a id="section-52g"></a>

### 12.11 怎样观察和验证网关缓存决策？

客户端可观察 `X-Cache-Hit: MISS/HIT`、`X-Cache-Layer: OpenResty-L1` 和 `X-Cache-Version`。读限流返回 429 与 `X-RateLimit-Layer: gateway-venue-read`，并写 `gateway.venue.rate_limited` 日志和共享计数；内部 purge 响应提供 `X-Gateway-Cache-Watermark`。Java Micrometer 暴露 `cache.caffeine.hit`、Redis hit/miss/error/bypassed、rebuild started/lock busy、DB fallback accepted/rejected、null marker hit 和 failure gate 状态。

[gateway-cache-consistency-test.ps1](./scripts/gateway-cache-consistency-test.ps1) 在真实 Java/OpenResty 环境验证 MISS→HIT、版本升级、v6 后收到 v5 purge 时水位不回退、响应版本低于水位时连续两次 MISS、公共读/匿名写语义，以及连续随机不存在 ID 出现 429。`VenueCacheInvalidatorTest` 覆盖两节点都成功与 A 成功/B 失败；`ReliableTaskSchedulerGatewayTest` 证明失败时不会调用 markDone。
...
`queryColdMiss` 对 `lock:venue:rebuild:{id}` 调用 Redisson `RLock.tryLock()`。没有传 lease time，因此使用 Redisson watchdog：owner 线程仍持锁且客户端存活时会自动续期，数据库查询超过旧实现的 10 秒也不会只因固定 TTL 到期而放入第二个正常重建者。`finally` 先判断 `isHeldByCurrentThread()` 再 unlock，不能误删后来者的锁。

获得锁后仍会二次读取 Redis，避免前一请求已经填好缓存而自己继续查库。未拿到锁时，每次等待配置的 25～50ms 并重读缓存，默认最多 20 轮。这里的正常重建只有持锁者能执行。

等待耗尽、等待期间 Redis 异常或加锁命令异常，不再无条件 `dbFallback.apply`，而是进入每 JVM 的无等待 `Semaphore` bulkhead。默认最多 8 个并发降级 DB 查询；拿不到 permit 立即抛 `CacheDegradedException`，由异常处理器返回 503；拿到者在 `finally` 释放 permit。正常 rebuild 与 degraded fallback 使用不同指标和日志。

旧 [unlock.lua](./src/main/resources/unlock.lua) 仍由基线 `CacheClient` 使用；Venue 主链已经不再用固定 10 秒 string lock。Redisson watchdog 只解决 owner 存活时的 lease 延续，不是 fencing，所以 cacheVersion 与版本水位仍然保留。

<a id="section-55"></a>

### 13.2 逻辑过期：旧值先返回，新值异步加载

RedisData 仍在但 expireTime 已到时，主分支先返回旧 Venue，再调用 `rebuildAsync`。同一 JVM 用 `localRebuilds` 把同一 key 合并成一个在途任务；后台线程再争取同一把 Redisson 分布式锁，并在锁内二次检查 Redis。因此单 JVM 不会为同一热点堆积大量任务，多 JVM 也只有持锁者真正查库。
...
逻辑期限决定何时刷新并允许一次旧读；物理期限让 Redis 键最终消失；随机增量分散到期；watchdog 锁压缩同 key 的正常回源；版本水位阻止较旧数据库快照重新覆盖 Redis。这些机制分别处理新鲜度、容量、集中到期、击穿和旧写顺序，不能互相替代。

Redis 调用现在明确产生 HIT、MISS 或 UNAVAILABLE：普通 MISS 不计为故障；命令异常累计到阈值后 failure gate 进入 OPEN，冷却期跳过 Redis 慢失败，之后只放一个 HALF_OPEN 探测，成功回到 CLOSED。Caffeine 命中始终在 gate 之前；Caffeine miss 才进入受控 DB 降级。逻辑过期是有效的 stale 数据，不会被误判为 Redis 故障，异步刷新失败也不会再额外查 DB。

| 配置 | 默认值 | 控制什么 |
|---|---:|---|
| `cache.reliability.redis-failure-threshold` | 3 | CLOSED 连续异常达到多少次后 OPEN |
| `cache.reliability.redis-open-duration-ms` | 3000 | OPEN 多久后允许一个 HALF_OPEN 探测 |
| `cache.reliability.db-fallback-max-concurrency` | 8 | 单 JVM 同时允许的降级 DB 查询数 |
| `cache.reliability.lock-wait-attempts` | 20 | 锁忙后的轮询次数 |
| `cache.reliability.lock-wait-min/max-delay-ms` | 25 / 50 | 每轮等待随机区间 |
| Lettuce / Redisson command timeout | 1 秒 | 单次 Redis 故障等待上界 |

冷 miss 的版本化写若被水位拒绝或 Redis 写失败，当前请求仍可拿到刚查到的数据库结果，但不会把它装入 Caffeine；正常回填和异步回填都只有 Redis 接受版本后才写本地。降级 DB 路径可以短期写 Caffeine，以便 Redis 故障期间后续同 JVM 请求继续服务，这是可用性与最长 10 分钟本地陈旧窗口之间的明确取舍。

<a id="section-57"></a>

### 13.4 CacheClient 哪些能力没有被 Venue 主链直接使用？

[CacheClient](./src/main/java/com/citypass/utils/CacheClient.java) 保留穿透、固定 TTL 互斥和逻辑过期工具；Venue 主查询使用 MultiLevelCacheService。两者的锁还刻意使用不同 namespace：基线是 `lock:venue:` string，主链是 `lock:venue:rebuild:` Redisson hash，避免两种 Redis 数据结构在同一 key 上冲突。
...
1. 检查搜索重建维护窗口是否允许相关写入；
2. `updateById` 更新业务字段；
3. `incrementCacheVersion(id)` 原子执行 cache_version=cache_version+1；
4. 重新读取 Venue，enqueue `INVALIDATE_VENUE_CACHE`；
5. 批量推进该 Venue 下活动的 searchVersion；
6. 按活动及版本 enqueue `INDEX_ACTIVITY_SEARCH`。

六步共用一个 MySQL 事务。任一步抛异常，Venue 变化、两类版本递增和任务记录一起回滚；成功提交后，即使应用立刻宕机，PENDING 任务仍保留。场馆关联活动很多时会同步生成同等数量的索引任务，这是当前简单扇出方案的写放大边界。
...
所有在线应用实例都通过 [RedisPubSubConfig](./src/main/java/com/citypass/config/RedisPubSubConfig.java) 订阅频道并清除自己的 Caffeine 项。发布任务的实例还会主动清本地项，避免依赖消息回环。最后把 Redis Lua 返回的实际水位带到每一个显式配置的 OpenResty purge 地址。

```mermaid
flowchart TD
    A["PUT /venues"] --> B["MySQL 事务：更新 Venue、两类 version、写缓存与搜索任务"]
    B --> C{"事务结果"}
    C -->|回滚| D["三项一起回滚"]
    ...
    F --> H["发布实例再次清本地项"]
    H --> I["逐个 OpenResty DELETE(id, version)"]
    I --> J["按任务 owner/version 标 DONE"]
    B --> K["INDEX_ACTIVITY_SEARCH：异步刷新关联活动 ES 文档"]
```

Redis Lua 返回空或任一网关 purge 失败都会抛异常，让任务进入退避重试或最终 DEAD。再次执行同一 cacheVersion 会保留 Redis 与各网关的较高水位、重复删除并重新 Publish，符合幂等重试需要。
...
| 冷 miss 正常回源 | Redis 同样拒绝低版本写入 | 只有 Lua 接受写入才 put；拒绝或写异常时 invalidate |
| Redis 故障的受控 DB 降级 | 无法读取共享水位 | 有 permit 的请求可 put，依赖 10 分钟本地 TTL 与恢复后的失效链 |
| null marker | 当前使用普通 SET | 不写 Caffeine，但未经过写入版本 Lua |

所以正常冷 miss 与异步重建都不会在 Redis 拒写后把旧对象写进 Caffeine；“所有路径都不可能留下旧本地对象”仍不成立，因为 Redis 故障降级无法读取水位，且 null marker / POST 新增协议仍未统一。项目选择用有限 permit 和本地 TTL 控制该可用性窗口。
...
OpenResty 现已支持显式配置多个 purge 地址，并要求全部返回成功；每个实例都用本地共享字典维护单调 cacheVersion 水位。它仍不会自动发现扩缩容后的实例，也没有把“某任务已由哪些节点确认”持久化到独立确认表，因此生产部署必须让配置覆盖所有独立 shared dict，或把地址指向能可靠广播 purge 的控制面。
...
| gateway-cache.purge-urls / 旧 purge-url | 空字符串 | app 的复数配置指向 `http://openresty/internal/cache/venue` |
...
## 17. 八种缓存故障窗口，逐个推演怎样恢复

<a id="section-73"></a>

### 17.1 Case 1：更新提交成功，失效任务正常完成

MySQL 已是新名称和新 cacheVersion，INVALIDATE 任务也已提交。Scheduler 领取后，Lua 推进水位、删除主/基线 Redis 键并广播；在线 JVM 清本地缓存，发布实例再主动清理，最后携带 version 逐个驱逐配置的网关。下一次 miss 回源新版本，旧网关响应低于水位时不能回填。
...
正常冷 miss 也接收版本化写入结果：Lua 返回 0 或 Redis 写异常时不会写 Caffeine。因此两个正常重建分支都受同一版本接受条件约束。

<a id="section-78"></a>

### 17.6 Case 6：网关驱逐失败

Redis 水位、删除和 Pub/Sub 已在 Lua 中完成，随后任一 HTTP purge 失败会抛出包含 endpoint 的异常。ReliableTaskScheduler 不会标 DONE，而是 markFailed 并退避；重试时 Lua 用同一版本再次删除并广播，再逐个尝试全部网关。

若连续失败达到 maxRetry，任务进入 DEAD。已成功收到 purge 的节点已推进水位；失败节点仍可能保留旧值到 TTL，因此需要依据日志中的 endpoint 修复并重放。任务状态表达的是整个 handler 是否完成，不是每个子步骤的独立状态。

<a id="section-78a"></a>

### 17.7 Case 7：Redis 整体不可用

Caffeine 已命中的请求不接触 Redis，仍可返回。Caffeine miss 的 Redis 连接异常与普通 MISS 分开记录；连续失败达到阈值后 gate OPEN，冷却期不让每个请求继续等待 Redis 超时。请求改走 DB fallback bulkhead，只有少量 permit 能查库，其余立即返回 503，所以 Redis 故障不会把全部流量平移到 MySQL。

冷却期结束后只允许一个 HALF_OPEN 探测。GET 成功即使结果是普通 MISS，也说明 Redis 命令通路恢复，gate 回到 CLOSED，后续请求重新使用互斥重建；探测失败则重新 OPEN。它是进程内轻量保护，不共享多 JVM 状态。

<a id="section-78b"></a>

### 17.8 Case 8：锁等待耗尽

持锁者可能仍在执行长查询，其他请求默认轮询约 0.5～1 秒。等待耗尽不表示 Redis 整体故障，也不表示可以放开查库；它只说明本次请求无法及时取得热点缓存结果。请求因此进入同一个 DB bulkhead：容量内允许有限降级，容量外 503，不排队。这样不能保证整个集群永远只有一个 DB 查询，但能把每个 JVM 的额外并发严格压在配置上限内。
...
flowchart TD
    ...
    CLAIM --> SIDE["MQ / 预约 Redis / Venue 失效 / 活动搜索索引"]
    SIDE --> RESULT{"围栏化结果"}
    RESULT -->|成功| DONE["DONE"]
    ...
    VINV --> GW["显式网关列表：DELETE(id, version)"]
    DB --> SOUT["活动或 Venue 搜索变化 + INDEX Outbox"]
    SOUT --> ES["external_gte 写 Elasticsearch"]
    DB --> CANAL["可选 Binlog → Canal → MQ"]
    CANAL --> VINV
    RATE["OpenResty Venue IP 令牌桶"] --> READ["Caffeine → Redis → watchdog 重建"]
    READ --> GATE["Redis failure gate"]
    GATE --> BULK["每 JVM DB fallback bulkhead"]
    BULK -->|permit| DB
    BULK -->|reject| FAIL["503"]
    DB --> READ
    READ --> VINV
```

预约、Venue 缓存和活动搜索都把关键外部意图持久化到任务表，但副作用语义不同：预约 Lua 维护数量和资源归属，Venue 失效删除可重建副本并广播，搜索任务把 MySQL 最新事实投影到可重建 ES 索引。任务租约保证可接管，执行版本保护任务表状态；外部正确性仍分别依赖幂等 marker、resourceVersion、cacheVersion 水位或 searchVersion 的单调写入。
...
**活动发布：** `ActivityPassServiceImpl.addLimitedPassStock / addActivityPass` → 保存活动；限量活动同时保存库存基准并写 INIT_RESERVATION_STOCK；两类活动都写 INDEX_ACTIVITY_SEARCH → 同事务提交。

**Venue 更新：** `VenueServiceImpl.update` → 更新字段、cacheVersion+1、关联活动 searchVersion+1 → INVALIDATE_VENUE_CACHE + 每活动 INDEX_ACTIVITY_SEARCH → 同事务提交。

<a id="section-83"></a>

### 20.2 可靠任务执行链

`ReliableTaskScheduler.relay` → 循环 `claimReady(1, ..., includeSearchTasks)` → 短事务 `FOR UPDATE SKIP LOCKED` → RUNNING + owner + lease + version → 事务外按七种类型执行 → `markDone`；异常时 `markFailed` 围栏回写 PENDING 或 DEAD。搜索关闭时只跳过 INDEX_ACTIVITY_SEARCH，不把它误标 DONE。

CREATE 与 TIMEOUT 都要求 RocketMQ 返回 SEND_OK；Redis 脚本负码抛错；Venue 网关 purge 失败也抛错；搜索 bulk 会检查单项失败。任务丢失租约时，旧 worker 的状态更新影响 0 行。
...
**读：** OpenResty Venue IP 令牌桶 → `VenueController.queryVenueById` → `VenueServiceImpl.queryById` → `MultiLevelCacheService` → Caffeine → Redis HIT/MISS/UNAVAILABLE。MISS 用 Redisson watchdog 锁正常重建；UNAVAILABLE 或等待耗尽经过 DB fallback bulkhead；正常回填再执行 `write-cache-if-version.lua`。

**应用写：** `PUT /venues` → MySQL 更新 + cacheVersion 递增 + INVALIDATE Outbox → Scheduler → `invalidate-venue-cache.lua` → Redis Pub/Sub → 在线 JVM 本地驱逐 → 多网关 `DELETE(id, version)`。
...
| 执行器 | [ReliableTaskScheduler.java](./src/main/java/com/citypass/task/ReliableTaskScheduler.java) | 逐条领取、七类分发、搜索关闭时保留 INDEX 任务、SEND_OK |
...
| 搜索索引任务 | [ActivitySearchIndexTaskHandler.java](./src/main/java/com/citypass/search/ActivitySearchIndexTaskHandler.java) | 回读事实、searchVersion 与 external_gte |
| 多级缓存 | [MultiLevelCacheService.java](./src/main/java/com/citypass/utils/MultiLevelCacheService.java) | HIT/MISS/UNAVAILABLE、watchdog、SWR、版本回填、DB bulkhead |
| 缓存可靠性配置 | [CacheReliabilityProperties.java](./src/main/java/com/citypass/config/CacheReliabilityProperties.java) | failure threshold、OPEN 时长、fallback 并发与等待参数 |
| Redis failure gate | [RedisFailureGate.java](./src/main/java/com/citypass/utils/RedisFailureGate.java) | CLOSED/OPEN/HALF_OPEN 与单探测恢复 |
| 缓存指标 | [CacheReliabilityMetrics.java](./src/main/java/com/citypass/utils/CacheReliabilityMetrics.java) | 各层命中、Redis 异常、重建与降级 Counter/Gauge |
...
| 网关 | [nginx.conf](./src/main/resources/openresty/nginx.conf) | Venue IP 读限流、响应缓存、TTL、内部 purge |
| 验证脚本 | [reliability-test.ps1](./scripts/reliability-test.ps1)、[cache-consistency-test.ps1](./scripts/cache-consistency-test.ps1)、[gateway-cache-consistency-test.ps1](./scripts/gateway-cache-consistency-test.ps1) | 故障恢复、双 JVM、旧版本拒写与随机 ID 限流 |
...
> 例如取消订单时，旧订单关闭、请求事实、库存恢复与 RESTORE_REDIS_STOCK 一起提交；Venue 更新时，业务字段、cacheVersion、关联活动 searchVersion 与缓存/搜索任务一起提交。若任务 INSERT 失败，业务也回滚；成功提交后，即使进程立刻退出，其他实例仍能读取任务。它保存的是恢复意图，并不表示外部副作用已经完成。
...
> 我称它为类 Transactional Outbox 或本地可靠任务表。CREATE、TIMEOUT、库存恢复、归属迁移、初始化、Venue 失效和活动搜索索引都通过一张表调度。项目实现了领取、租约、重试和 DEAD，但没有通用事件 schema、订阅者治理或全局顺序平台，所以不会宣称是完整事件总线。
...
> 没有。任务执行前用户仍可能命中旧缓存，逻辑过期也允许本次返回旧值；Pub/Sub 没有逐实例持久确认，网关列表也依赖静态配置覆盖真实拓扑。Redis 与 OpenResty 能拒绝低于水位的正常回填，但 Redis 故障降级无法读取共享水位，可能在当前 JVM 暂存数据库结果。项目实现的是有容量边界的可用性与最终一致性，不是所有层同步强一致。

<a id="section-96"></a>

### 21.9 多级缓存最大的复杂度是什么？

**先理解：** OpenResty、每个 JVM 和 Redis 都能独立保存副本；除了旧写顺序，还要防穿透、热点击穿和缓存故障把流量级联到 MySQL。

**30～60 秒回答：**

> 一致性上，我用失效 Outbox、Redis 版本水位、Pub/Sub 和网关 version+body 原子项控制传播与旧回填。可靠性上，null marker 保护重复空 key，OpenResty IP 令牌桶约束随机 key；Redisson watchdog 锁串行正常重建，等待耗尽和 Redis 故障统一经过每 JVM DB bulkhead。Redis 连续异常会短暂 OPEN，HALF_OPEN 探测成功后恢复正常链路。边界仍包括异步可见窗口、Pub/Sub 无持久确认、静态网关拓扑与新增 Venue 协议。
...
| ReliableTask 就是完整事件平台 | 它是同事务保存并执行七类副作用的本地任务表 |
...
| null marker 能阻止任意随机 ID 穿透 | 它只保护重复 key；随机 key 由独立的 OpenResty IP 读令牌桶约束 |
| Redis MISS 与连接异常都是“没读到” | MISS 继续正常互斥重建，ERROR 进入 failure gate 与受控降级 |
| Venue 主链还是 10 秒 SETNX 锁 | 主链已用无固定 lease 的 Redisson watchdog 锁；旧锁仅留在基线 CacheClient |
| 等锁超时或 Redis 挂了就全部直查 DB | 每 JVM Semaphore 无等待限并发，超容量直接 503 |
...
| 版本水位杜绝任何旧对象进入 Caffeine | 正常重建拒写后不 put；Redis 故障降级无法读取水位，仍有本地 TTL 窗口 |
...
| 简历表述 | 对应实现 | 面试时带上的边界 |
|---|---|---|
| 类 Transactional Outbox | 请求、订单、库存、Venue、活动搜索事实与对应任务同事务 | 七类工作，不包装成通用事件平台 |
...
| OpenResty + Caffeine + Redis | 网关 version+正文、LocalEntry、RedisData | 各层 TTL、版本水位与实例范围不同 |
| 穿透与击穿防护 | IP 读令牌桶、null marker、Redisson watchdog、双重检查 | 不是完整 WAF；降级额外查库受 bulkhead 限制 |
| Redis 故障受控降级 | failure gate + 每 JVM Semaphore + 503 | 不承诺无损；状态不跨 JVM，共享 DB 总容量需部署侧核算 |
| 持久化缓存失效 | Venue cacheVersion 与 INVALIDATE 任务同事务 | 异步最终一致，不承诺读后立即可见 |
| 多 JVM 失效广播 | Redis Lua Publish + 每实例 listener | 在线广播、没有逐实例确认 |
| 版本化防旧写 | 失效水位 + write-cache-if-version.lua | 正常冷 miss/异步回填成立；故障降级有本地窗口 |
...
> MySQL、Redis、MQ 和 Elasticsearch 不能由普通本地事务一起原子提交。我先保证订单、请求、候补、库存、Venue 或活动搜索事实与相应任务在 MySQL 同事务落库，再异步执行 CREATE、TIMEOUT、库存恢复、归属迁移、库存初始化、缓存失效或搜索索引更新。
>
...
>
> Venue PUT 在事务中更新业务字段、递增 cacheVersion 并写 INVALIDATE 任务。执行时，Redis Lua 先把 version 水位单调推进，删除主缓存和独立压测基线，再 Publish id:version。所有在线 JVM listener 清本地 Caffeine，发布实例也主动清理，最后携带实际 version 逐个 purge 显式配置的 OpenResty。Canal 可从 binlog 触发同一条失效链，覆盖绕过应用的写入。
>
> 回填时，write-cache-if-version.lua 拒绝低于 Redis 水位的数据库快照；逻辑过期异步重建只有写 Redis 成功才写本地缓存。网关侧把 version 与 body 存为一个值，在场馆锁内比较本地水位，因此 v4 慢响应晚于 v5 purge 到达时会正常返回给原请求但不能复活网关缓存。
>
> 读侧还把普通 MISS 与 Redis ERROR 分开：冷 miss 用 Redisson watchdog 锁和双重检查，锁等待耗尽或 Redis 故障只能经过每 JVM DB bulkhead，超容量返回 503；failure gate 在连续异常后短暂绕过 Redis，HALF_OPEN 成功后自动恢复。网关 IP 令牌桶限制随机不存在 ID，Redis null marker 保护重复不存在 ID。
>
> 这仍是最终一致：失效任务执行前可能读旧，Pub/Sub 没有逐实例 ack，多网关依赖静态配置覆盖真实拓扑；故障降级写 Caffeine 与 POST 新增 Venue 也还有边界。我会把已实现的版本协议、容量保护与这些限制一起说明。
...
flowchart TD
    ...
    F --> F1["穿透：null marker 与 IP 限流怎样分工？"]
    F --> F2["击穿：watchdog、等待与 bulkhead 怎样分工？"]
    F --> F3["Redis ERROR 怎样 OPEN、探测和恢复？"]
```
...
| IP 读限流只按来源且状态在单网关内 | 生产环境可接入边缘限流、可信代理 IP 解析与跨节点策略 |
| failure gate 与 DB bulkhead 都是每 JVM | 根据实例数和 MySQL 总容量统一核算上限，必要时增加全局准入 |
| 降级 DB 结果可写 Caffeine，Redis 故障时无法校验水位 | 使用更短的降级专用 TTL，或恢复后主动核验/驱逐本地代际 |
| Redis Pub/Sub 没有逐 JVM 持久确认 | 按一致性要求增加可观察的消费位点或拉取版本校验 |
| 多网关地址需静态枚举，没有自动发现与逐节点持久确认 | 接入服务发现或独立失效控制面，记录实例拓扑与确认位点 |
...
可靠任务部分重新核查了 DDL、Java 映射、七种 taskType、全部 enqueue/enqueueAt、搜索关闭时的领取过滤、逐条调度、owner/lease/version 围栏、退避/DEAD/replay、Micrometer、SEND_OK、三个预约 Lua 与对账三阶段。活动搜索任务的版本、重建和专项验收见独立搜索学习文档。

缓存部分核查了 Venue cacheVersion 事务、失效任务、两个 Redis 版本 Lua、Redis Pub/Sub listener、多 JVM Caffeine 范围、OpenResty 原子缓存项与版本水位、多网关 purge、Canal、独立基线键、RedisData/LocalEntry、冷 miss、逻辑过期和负缓存。
...
2026-09-19 最终执行 `mvn clean package`：40 个测试，0 failure、0 error，2 个既有环境测试 skipped；真实 OpenResty 镜像 `nginx -t` 通过，[cache-consistency-test.ps1](./scripts/cache-consistency-test.ps1) 与 [gateway-cache-consistency-test.ps1](./scripts/gateway-cache-consistency-test.ps1) 也通过。两个脚本使用真实 MySQL、Redis、RocketMQ、Java 与 OpenResty，并临时启动第二个 JVM，不是只检查模拟对象。

可靠性脚本覆盖：100 个并发用户争抢 10 份库存、连续两次释放补位两人、锁住业务队首时后续候选不跳号、Broker 故障后的 CREATE 恢复、支付/超时竞争只产生一个终态，以及恢复 Lua 首次/重放返回 1/0。

`MultiLevelCacheServiceTest` 记录的实际调用是：200 个同 key 冷 miss 只执行 1 次 DB；10.2 秒慢查询下两个请求仍只有 1 个正常重建；锁等待耗尽的 32 个请求中 2 个进入降级 DB、30 个快速拒绝；Redis 不可用的 32 个 Caffeine miss 同样为 2/30，而已预热的 Caffeine hit 不访问 DB；恢复后的 HALF_OPEN 探测重新关闭 gate。逻辑过期 100 并发都先返回 stale，只安排 1 次 DB 重建；null marker 第二次读取不查 DB。另用真实 Redis 7.2 和两个 Redisson 3.13.6 客户端把 watchdog 周期缩为 3 秒：持锁 10.2 秒后 TTL 仍有 2660ms，第二客户端不能加锁；owner unlock 后才能成功。

缓存脚本真实启动第二个应用实例，验证两个 JVM 预热后，PUT 更新能让两边最终读到新名称、MySQL 与 Redis 版本水位为 3/3，并验证旧版本不能覆盖新水位。网关专项脚本验证 MISS/HIT、版本升级、乱序 purge、旧响应拒写和权限语义；将令牌桶临时配置为每秒 1、突发 3 后，20 个随机不存在 ID 中 17 个返回 429。Java 测试验证 503 映射、多地址全成功与单点失败时的任务状态；Actuator 可查询全部 11 个缓存可靠性指标。仍未故障注入网络分区漏收 Pub/Sub、长期 DEAD、动态网关拓扑或多 JVM 同时耗尽各自 DB bulkhead。

<a id="section-108"></a>

### 26.3 文档格式校验

**格式检查**：35 个不同的相对文件目标均从仓库根目录核实存在；完整目录覆盖 118 个章节和小节，118 个目录锚点均可解析，代码围栏已检查；14 张 Mermaid 图均通过 Mermaid CLI 11.17.0 语法解析。
...
应能先定位旧值在哪一层，再解释：更新、cacheVersion 与 INVALIDATE 同事务；失效 Lua 推进版本水位、删两个 Redis 键并 Publish；在线 JVM 清 Caffeine；各网关用同一 version 单调推进水位；旧 Redis、正常重建和网关慢响应分别受版本限制；最后指出任务执行窗口、Pub/Sub、故障降级本地窗口、POST 新增和静态网关拓扑边界。
